...
import top.gate.boxcore.entity.pm.InstalledPackage;
import top.gate.boxcore.server.BProcessManager;
...
import top.gate.boxcore.utils.AbiUtils;
import top.gate.boxcore.utils.FileUtils;
import top.gate.boxcore.utils.Slog;
import top.gate.boxcore.utils.compat.ObjectsCompat;
import top.gate.boxcore.utils.compat.PackageParserCompat;
...
public class BPackageManagerService extends IBPackageManagerService.Stub implements ISystemService {
    ...
    public ApplicationInfo getApplicationInfo(String packageName, int flags, int userId) {
        ...
        if (ObjectsCompat.equals(packageName, BBoxCore.getHostPkg())) {
            try {
                return BBoxCore.getPackageManager().getApplicationInfo(packageName, flags);
    ...
    public PackageInfo getPackageInfo(String packageName, int flags, int userId) {
        ...
        if (ObjectsCompat.equals(packageName, BBoxCore.getHostPkg())) {
            try {
                return BBoxCore.getPackageManager().getPackageInfo(packageName, flags);
            } catch (PackageManager.NameNotFoundException e) {
                e.printStackTrace();
            }
            return null;
    ...
    private InstallResult installPackageAsUserLocked(String file, InstallOption option, int userId) {
        ...
        try {
            ...
            if (option.isFlag(InstallOption.FLAG_URI_FILE)) {
                ...
                InputStream inputStream = BBoxCore.getContext().getContentResolver().openInputStream(Uri.parse(file));
            ...
            if (!support) {
                return result.installError(BBoxCore.is64Bit() ? "not support armeabi-v7a abi" : "not support arm64-v8a abi");
            }
