...
import top.gate.boxcore.client.frameworks.BXposedManager;
import top.gate.boxcore.client.hook.HookManager;
import top.gate.boxcore.client.hook.IOManager;
import top.gate.boxcore.client.hook.env.VirtualRuntime;
import top.gate.boxcore.client.hook.fixer.ContextFixer;
import top.gate.boxcore.client.hook.proxies.app.HCallbackStub;
import top.gate.boxcore.client.hook.proxies.context.providers.ContentProviderStub;
import top.gate.boxcore.BBoxCore;
import top.gate.boxcore.client.hook.delegate.ActivityLifecycleDelegate;
...
import top.gate.boxcore.entity.ClientConfig;
import top.gate.boxcore.entity.pm.InstalledModule;
import top.gate.boxcore.server.ClientServiceManager;
import top.gate.boxcore.utils.FileUtils;
import top.gate.boxcore.utils.Slog;
import top.gate.boxcore.utils.compat.ActivityManagerCompat;
import top.gate.boxcore.utils.compat.BuildCompat;
import top.gate.boxcore.utils.compat.StrictModeCompat;
...
public class BClient extends IBClient.Stub {
    ...
    public Service createService(ServiceInfo serviceInfo) {
        ...
        try {
            Context context = BBoxCore.getContext().createPackageContext(
    ...
    public synchronized void handleBindApplication(String packageName, String processName) {
        ...
        PackageInfo packageInfo = BBoxCore.getBPackageManager().getPackageInfo(packageName, PackageManager.GET_PROVIDERS, BClient.getUserId());
        ...
        Object boundApplication = ActivityThread.mBoundApplication.get(BBoxCore.mainThread());
        ...
        try {
            ...
            ActivityThread.mInitialApplication.set(BBoxCore.mainThread(), mInitialApplication);
            ContextFixer.fix((Context) ActivityThread.getSystemContext.call(BBoxCore.mainThread()));
    ...
    private Context createPackageContext(ApplicationInfo info) {
        try {
            return BBoxCore.getContext().createPackageContext(info.packageName, Context.CONTEXT_INCLUDE_CODE | Context.CONTEXT_IGNORE_SECURITY);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return null;
            ...
            for (ProviderInfo providerInfo : provider) {
                try {
                    if (processName.equals(providerInfo.processName)) {
                        ActivityThread.installProvider(BBoxCore.mainThread(), context, providerInfo, null);
                    }
                } catch (Throwable ignored) {
                }
            }
            ...
            for (InstalledModule installedModule : installedModules) {
                ...
                try {
                    XposedCompat.loadModule(installedModule.getApplication().sourceDir,
                            ...
                            BBoxCore.getContext().getClassLoader());
                } catch (Throwable e) {
                    e.printStackTrace();
                }
            }
    ...
    public IBinder getActivityThread() {
        return ActivityThread.getApplicationThread.call(BBoxCore.mainThread());
    }
    ...
    public IBinder acquireContentProviderClient(ProviderInfo providerInfo) throws RemoteException {
        ...
        ContentProviderClient contentProviderClient = BBoxCore.getContext()
        ...
        if (iInterface == null)
            return null;
        IInterface proxyIInterface = new ContentProviderStub().wrapper(iInterface, BBoxCore.getHostPkg());
    ...
    public void registerReceivers(Application application) {
        try {
            ...
            List<ResolveInfo> resolves = BBoxCore.getBPackageManager().queryBroadcastReceivers(intent, PackageManager.GET_RESOLVED_FILTER, null, BClient.getUserId());
...
//                        e.printStackTrace();
    ...
    public void finishActivity(final IBinder token) {
        mH.post(new Runnable() {
            @Override
            public void run() {
                Map<IBinder, Object> activities = ActivityThread.mActivities.get(BBoxCore.mainThread());
        ...
        mH.post(new Runnable() {
            ...
            public void run() {
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP_MR1) {
                    newIntent = ReferrerIntent.ctor.newInstance(intent, BBoxCore.getHostPkg());
                } else {
                    newIntent = intent;
                }
                if (ActivityThread.performNewIntents != null) {
                    ...
                    ActivityThreadQ.handleNewIntent.call(BBoxCore.mainThread(), token, Collections.singletonList(newIntent));
                }
            }
        });
    }
