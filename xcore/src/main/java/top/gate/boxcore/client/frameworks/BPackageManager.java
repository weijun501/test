...
import top.gate.boxcore.server.pm.IBPackageManagerService;
...
public class BPackageManager {
    ...
    public Intent getLaunchIntentForPackage(String packageName, int userId) {
        ...
        List<ResolveInfo> ris = queryIntentActivities(intentToResolve,
                0,
                intentToResolve.resolveTypeIfNeeded(BBoxCore.getContext().getContentResolver()),
                userId);
    ...
    private IBPackageManagerService getService() {
        ...
        mService = IBPackageManagerService.Stub.asInterface(BBoxCore.get().getService(ServiceManager.PACKAGE_MANAGER));
        return getService();
    }
}
