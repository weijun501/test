...
import top.gate.boxcore.client.record.service.ServiceRecord;
import top.gate.boxcore.entity.UnbindRecord;
import top.gate.boxcore.client.stub.record.StubServiceRecord;
...
//        Log.d(TAG, "onStartCommand: " + component.toString());
        ...
        try {
            ...
            BBoxCore.getBActivityManager().onStartCommand(proxyIntent, stubRecord.mUserId);
            return i;
        } catch (Throwable e) {
            e.printStackTrace();
        }
...
//        Log.d(TAG, "onLowMemory");
    ...
    public boolean onUnbind(Intent proxyIntent) {
        ...
        try {
            UnbindRecord unbindRecord = BBoxCore.getBActivityManager().onServiceUnbind(proxyIntent, BClient.getUserId());
            ...
            if (destroy || record.decreaseConnectionCount(intent)) {
                ...
                if (destroy) {
                    service.onDestroy();
                    BBoxCore.getBActivityManager().onServiceDestroy(proxyIntent, BClient.getUserId());
