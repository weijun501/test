package top.gate.boxcore.client.hook.delegate;
...
import top.gate.boxcore.client.hook.proxies.context.providers.SettingsProviderStub;
...
//        ContentProviderHolderOreo.provider.set(holder, (IInterface) contentProviderDelegate.getProxyInvocation());
        ...
        switch (auth) {
            case "settings":
                vContentProvider = new SettingsProviderStub().wrapper(iInterface, BBoxCore.getHostPkg());
                break;
            default:
                vContentProvider = new ContentProviderStub().wrapper(iInterface, BBoxCore.getHostPkg());
                break;
        }
        if (BuildCompat.isOreo()) {
    ...
    public static void init() {
        ...
        BBoxCore.getContext().getContentResolver().call(Uri.parse("content://settings"), "", null, null);
        Object activityThread = BBoxCore.mainThread();
...
//                if (iInterface instanceof Proxy)
//                    continue;
                ActivityThread.ProviderClientRecordP.mProvider.set(value, new ContentProviderStub().wrapper(iInterface, BBoxCore.getHostPkg()));
