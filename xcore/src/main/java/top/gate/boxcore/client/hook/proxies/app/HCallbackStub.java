...
import top.gate.boxcore.client.stub.record.StubActivityRecord;
    ...
    private boolean handleLaunchActivity(Object client) {
        ...
        if (activityInfo != null) {
            if (BClient.getClientConfig() == null) {
                BBoxCore.getBActivityManager().restartProcess(activityInfo.packageName, activityInfo.processName, stubRecord.mUserId);
                return true;
            }
            ...
            BBoxCore.getBActivityManager().onActivityCreated(taskId, token, stubRecord.mActivityRecord);
